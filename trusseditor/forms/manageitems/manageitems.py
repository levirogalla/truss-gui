...
from ..jointmenu.editcoordinates import EditCoordinates
...
class TrussItems(QDialog):
    ...
    def addForce(self):
        force = ForceForm.get_force(self.truss.joints, None)
        if force is None:
            QMessageBox.warning(
    ...
    def addSupport(self):
        joint, support_type = ForceForm.get_force(self.truss.joints, None)
    ...
    def get_selected_items(self):
        ...
        for item_cell in selected_items:
            ...
            if row not in rows_visted:
                item_id = int(self.ui.connectedItems.item(row, 0).text())
                print(self.truss_widget.connections)
                item = self.parent(
                ).connections[item_id]
                rows_visted.add(row)
                yield item
